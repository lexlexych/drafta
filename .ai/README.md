...
2. `/startEpic E-001` → оркестратор создаёт (или переиспользует) git-ветку `E-001`
   и по каждому тикету запускает разработчика, затем ревьюера; вся работа коммитится
   в ветку эпика; статусы обновляются после каждого перехода; при замечаниях —
   до 2 циклов доработки, дальше `blocked` и стоп.
3. Все dev-тикеты `done` → эпик в статусе `manual_steps`: человек проверяет результат
   и выполняет шаги из `T-NN-executive-summary.md` (настройка Supabase, секреты и т.п.).
4. `/finishEpic E-001` → статусы `done`, ветка `E-001` вливается в `main`
   (merge `--no-ff`); ветку удалять и пушить — решает человек.

Прерывание не страшно: `/startEpic` всегда продолжает с первого незавершённого тикета.
Git-политика целиком — в `.ai/workflow.md`.
