...
После применения миграций к локальной Supabase запустите `pnpm dev`.
Workflow запускается внутри Next.js; отдельный сервер очереди не нужен.
Наблюдение: `pnpm exec workflow inspect`. Production использует fra1 и два
защищённых Cron-маршрута. Подробности: [Vercel Workflow](docs/integrations/vercel-workflow.md).
