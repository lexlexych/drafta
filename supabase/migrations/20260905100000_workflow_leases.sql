...
--
-- Inngest давал лимиты конкурентности декларативно: у каждой функции стоял
-- `concurrency: [{ key: '"workspace:" + event.data.workspaceId', limit: 2 }]`,
-- и платформа сама держала очередь. В Workflow SDK такого примитива нет:
-- нативный `lock()` заявлен в план v6, а до него официальная рекомендация —
-- «принесите свою конкурентность» (собственный семафор поверх общего хранилища).
-- Единственное общее хранилище проекта — Supabase, поэтому семафор живёт здесь.
--
...
comment on table public.workflow_leases is
  'Семафор конкурентности workflow-прогонов: замена concurrency-ключей Inngest до появления нативного lock() в Workflow SDK. Пишется только под service role.';
