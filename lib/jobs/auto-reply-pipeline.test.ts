...
/**
 * Шаги без Workflow: запоминают порядок, а «сон» просто записывает дедлайн.
...
describe("runAutoReplyPipeline", () => {
  it("waits, classifies, sends the template and journals the decision", async () => {
    ...
    expect(result).toEqual({ outcome: "queued", replyMessageId: "out-1" });
  ...
  it("skips the sleep step entirely when the delay is zero", async () => {
    // Нулевая длительность у Workflow не документирована, поэтому «отвечать
