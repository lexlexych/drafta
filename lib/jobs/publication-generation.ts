...
export async function runPublicationGeneration(input: {workspaceId:string;jobId:string}, step:LocalSteps) {
  const { workspaceId, jobId } = input;
  const assetId = await step.run("asset-id", () => randomUUID());
  // All content remains inside step callbacks. Only IDs are returned to Workflow.
