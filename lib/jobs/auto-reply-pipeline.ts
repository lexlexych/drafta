 ...
 *
 * Чистый пайплайн с инжектируемыми шагами — как `./send-pipeline.ts`: Workflow
...
export async function runAutoReplyPipeline(
  ...
  for (;;) {
    ...
    // длительность в Workflow не документирована. Дедлайн в прошлом (прогон
    // стартовал с задержкой) Workflow пропускает сам.
...
export const autoReplyDependencies: AutoReplyDependencies = {
  ...
  requestSend: async (input) => {
    const { emitMessageSendRequested } = await import("@/lib/workflows/events");
