...
describe("publication delivery safety",()=>{
  ...
  it("never repeats an uncertain send even inside the provider idempotency window",async()=>{job.media=[{type:"image",url:"https://storage.test/saved",path:"saved"}];job.first_sent_at=new Date().toISOString();await runPublicationDelivery("workspace","delivery");expect(mocks.publish).not.toHaveBeenCalled();expect(job.status).toBe('uncertain');expect(uploads).toHaveLength(0);});
  it("checks a failed remote post without retrying it",async()=>{job.remote_id='remote';job.check_only=true;mocks.get.mockResolvedValue({remoteId:'remote',status:'failed'});await runPublicationDelivery('workspace','delivery');expect(mocks.get).toHaveBeenCalledOnce();expect(mocks.retry).not.toHaveBeenCalled();expect(mocks.publish).not.toHaveBeenCalled();});
  it("never creates a post from a status-only request without a remote id",async()=>{job.check_only=true;await runPublicationDelivery('workspace','delivery');expect(mocks.publish).not.toHaveBeenCalled();expect(job.status).toBe('uncertain');});
