...
export type CommentDraftsPipelineInput = {
  workspaceId: string;
  postId: string;
...
export type CommentDraftsPipelineResult =
...
export type CommentDraftsSteps = {
...
export type LoadedCommentDraftsContext = {
  ...
  aiSettings: CommentPipelineAiSettings;
...
export type CommentDraftsDependencies = {
  loadContext(
    input: CommentDraftsPipelineInput,
  ): Promise<LoadedCommentDraftsContext | { skip: CommentDraftsPipelineResult }>;
  startDraft(input: {
    workspaceId: string;
    commentId: string;
    kbFileIds: string[];
...
export const commentDraftsDependencies: CommentDraftsDependencies = {
  loadContext,
  startDraft,
...
export async function runCommentDraftsPipeline(
  input: CommentDraftsPipelineInput,
  steps: CommentDraftsSteps,
  dependencies: CommentDraftsDependencies = commentDraftsDependencies,
  logger?: PromptLogger,
): Promise<CommentDraftsPipelineResult> {
  ...
  // Grows as the run proceeds: each prompt sees every reply drafted before it.
  const siblingDraftTexts = [...context.existingDraftTexts];
  let generated = 0;

  for (const [index, target] of context.targets.entries()) {
    const draftId = await steps.run(`start-draft:${index}`, () =>
      dependencies.startDraft({
    ...
    if (!draftId) {
      // The comment disappeared (post deleted, comment removed) between loading
      // the context and starting its draft — skip it, keep the rest of the run.
      continue;
    }

    const completion = await steps.run(`generate:${index}`, async () => {
    ...
    await steps.run(`finalize:${index}`, () =>
