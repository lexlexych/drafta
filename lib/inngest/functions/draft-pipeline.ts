...
/** Enough of the interaction to decide debounce vs. immediate processing. */
...
export type DraftPipelineDependencies = {
  ...
  finalizeDraft(input: {
    ...
    /** Comment threads keep one draft per comment — supersede only same-comment drafts. */
    commentScoped: boolean;
  }): Promise<void>;
...
function normalizeAiSettings(row: Record<string, unknown>): PipelineAiSettings {
  if (
    ...
    typeof row.auto_generate_comments !== "boolean"
  ) {
  ...
  return {
    ...
    autoGenerateComments: row.auto_generate_comments,
  };
}
...
/**
 * Comment threads are not debounced (docs/architecture/07-data-flows.md#правила-поверх-дебаунса:
 * «для kind = comments … дебаунс не нужен … классификация … поштучно»). Each
 * incoming comment gets its own draft targeting exactly that comment. The
 * context handed to the prompt is the thread up to and including the target
 * comment (so the target is the latest incoming to answer); newer comments —
 * which are handled by their own runs — are excluded.
 */
export function selectCommentContext(
  chronologicalMessages: readonly PipelineMessage[],
  targetMessageId: string | undefined,
): { contextMessages: PipelineMessage[]; batchMessages: PipelineMessage[] } | null {
  const targetIndex = targetMessageId
    ? chronologicalMessages.findIndex(
        (message) =>
          message.id === targetMessageId && message.direction === "incoming",
      )
    : findLastIncomingIndex(chronologicalMessages);

  if (targetIndex < 0) {
    return null;
  }

  const target = chronologicalMessages[targetIndex]!;
  return {
    contextMessages: chronologicalMessages.slice(0, targetIndex + 1),
    batchMessages: [target],
  };
}

function findLastIncomingIndex(
  chronologicalMessages: readonly PipelineMessage[],
): number {
  for (let index = chronologicalMessages.length - 1; index >= 0; index -= 1) {
    if (chronologicalMessages[index]!.direction === "incoming") {
      return index;
    }
  }
  return -1;
...
async function loadDebounceContext(input: {
  ...
  if (kind !== "dm" && kind !== "comments") {
    throw new Error("Conversation kind is unsupported.");
  }
  ...
  return { kind, debounceSeconds: normalizeDebounceSeconds(settings.debounce_seconds) };
}
...
function postTextFromMetadata(postMetadata: unknown): string {
  if (typeof postMetadata !== "object" || postMetadata === null) {
    return "";
  }
  const text = (postMetadata as Record<string, unknown>).text;
...
async function loadContext(
  ...
  if (conversationKind !== "dm" && conversationKind !== "comments") {
    throw new Error("Conversation kind is unsupported.");
  }
  ...
  const [
    ...
    supabase
      .from("ai_settings")
      .select(
        "tone, language, signature, debounce_seconds, model, auto_generate_dm, auto_generate_comments",
      )
      .eq("workspace_id", input.workspaceId)
      .maybeSingle(),
    supabase
  ...
  // DM answers the whole un-replied batch after the debounce; a comment thread
  // answers one specific comment (docs/architecture/07-data-flows.md#правила-поверх-дебаунса).
  let messages: PipelineMessage[];
  let batchMessages: PipelineMessage[];
  if (conversationKind === "comments") {
    const commentContext = selectCommentContext(
    ...
    messages = commentContext.contextMessages;
    batchMessages = commentContext.batchMessages;
  } else {
    messages = chronologicalMessages;
    batchMessages = selectBatchMessages(messages);
  }
...
async function finalizeDraft(input: {
  ...
  const { data, error } = await supabase.rpc("finalize_draft_generation", {
    ...
    comment_scoped: input.commentScoped,
  });
...
export async function runDraftPipeline(
  ...
  if (!input.regenerate) {
    ...
    // Comment threads are not debounced — each comment is self-contained and
    // answered on its own (docs/architecture/07-data-flows.md#правила-поверх-дебаунса).
    // Only DM waits out the debounce window and lets the newest event win.
    if (debounceContext.kind === "dm") {
  ...
  const stopReason = await steps.run("stop-check", () => {
    // Separate auto-generation toggles for DM and comments
    // (docs/architecture/06-data-model.md#ai_settings).
    const autoGenerate =
      context.conversationKind === "comments"
        ? context.aiSettings.autoGenerateComments
        : context.aiSettings.autoGenerateDm;
    if (!input.regenerate && !autoGenerate) {
  ...
  await steps.run("finalize", () =>
    dependencies.finalizeDraft({
      ...
      commentScoped: context.conversationKind === "comments",
    }),
  );
