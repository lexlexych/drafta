...
import { generatePublicationIdeas, generatePublicationText, generatePublicationImage, PublicationGenerationError, type PublicationUsage } from "@/lib/ai/publications";
import { validResult, type AuthoringInput, type AuthoringResult, type PostIdea, type PublicationVariant } from "@/lib/publications/authoring";

type Snapshot = { input: AuthoringInput; ideas: PostIdea[]; variants: PublicationVariant[]; assetIds: string[]; title: string };
...
export const publicationGeneration = inngest.createFunction({
    ...
    try {
      ...
      if (job.kind === "ideas") {
        ...
        if (job.kind === "image" && job.snapshot.variants.length) {
          await checkpoint(workspaceId, jobId, { result: { title: job.snapshot.title, variants: job.snapshot.variants, assetIds: [], imagePrompt: job.snapshot.input.brief.description || job.snapshot.input.brief.topic } });
        } else {
          await checkpoint(workspaceId, jobId, { stage: "text" });
          const generated = await generatePublicationText({ ...context, input: job.snapshot.input });
  ...
  await step.run("image", async () => {
    const job = await load(workspaceId, jobId); if (!job || job.kind === "ideas" || job.kind === "text" || job.snapshot.input.kind === "text" || job.result?.assetIds?.length) return;
    try {
      await loadSelectedContext(workspaceId, job.snapshot.input);
      await checkpoint(workspaceId, jobId, { stage: "image" });
      const input = job.snapshot.input;
      if (input.image.source === "upload" && input.image.assetId) {
        await imageBytes(workspaceId, job.draft_id, input.image.assetId);
        await checkpoint(workspaceId, jobId, { result: { ...job.result, assetIds: [input.image.assetId] } });
      } else {
        const reference = input.image.referenceId ? await imageBytes(workspaceId, job.draft_id, input.image.referenceId) : undefined;
        const generated = await generatePublicationImage(input, job.result?.imagePrompt ?? input.brief.topic, reference);
        await storeImage(workspaceId, job.draft_id, assetId, generated.bytes);
        await checkpoint(workspaceId, jobId, { result: { ...job.result, assetIds: [assetId] }, usage: [...job.usage,generated.usage] });
      }
    } catch (error) {
  ...
  await step.run("finish", async () => {
    const job = await load(workspaceId, jobId); if (!job || job.kind === "ideas") return;
