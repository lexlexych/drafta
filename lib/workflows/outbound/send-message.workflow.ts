 ...
 *
 * `try/catch` заменяет `onFailure` Inngest: когда ретраи шага исчерпаны,
 * ошибка доходит сюда, и сообщение помечается `failed` — в треде появляется
 * кнопка «Повторить» вместо вечно висящего `pending`.
 */
