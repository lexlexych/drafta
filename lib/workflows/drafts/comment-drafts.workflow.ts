 ...
 *
 * Лизы те же, что стояли в `concurrency` Inngest: бюджет LLM воркспейса
 * (лимит 2) и взаимоисключение по посту (лимит 1) — два прогона не должны
 * черновить один пост одновременно.
 *
 * `catch` заменяет `onFailure`: черновики, оставшиеся в статусе `generating`,
 * удаляются, иначе на экране навсегда зависла бы карточка со спиннером.
 */
