 ...
 * §6.2): действие эмитит событие, работу делает Workflow-функция
 * `generate-draft` с ретраями, а результат приезжает сюда через Realtime —
...
/**
 * Страховка на случай, когда прогон не доживает до статуса в базе: пока Workflow
 * доедает ретраи, `onFailure` ещё не сработал, а поле уже нельзя держать
