 ...
 * со значком мессенджера. Для Instagram, WhatsApp, Facebook и LinkedIn она раскрывает
 * короткий онбординг с предусловиями платформы и кнопкой «Войти через <канал>», которая
...
const CONNECT_PREREQUISITES: Readonly<Record<ChannelPlatform, string[]>> = {
  ...
  facebook: [
    "Подключается страница Facebook, а не личный профиль. Если страницы у бизнеса ещё нет, сначала создайте её в Facebook.",
    "Вы администратор этой страницы — с полным доступом к ней в Facebook или Meta Business Suite.",
    "В этом браузере вы уже вошли именно в тот аккаунт Facebook, через который управляете страницей, — иначе на следующем шаге легко подключить чужую.",
    "На шаге Meta оставьте все запрошенные разрешения включёнными: без них сообщения из Messenger и комментарии к постам не будут приходить в drafta.",
    "Затем выберите одну страницу — в drafta подключается одна страница Facebook.",
  ],
  linkedin: [
