...
import { DEFAULT_AUTHORING, EMPTY_IDEA, IDEA_FIELDS, generationIssue, ideaContextKey, type AuthoringInput, type AuthoringJob, type AuthoringResult, type AuthoringState, type ChannelChoice, type PostIdea } from "@/lib/publications/authoring";
...
export function PublicationWizard({ draftId, onClose }: { draftId: string; onClose: () => void }) {
  ...
  const active = !!data?.state.active_job_id; const frozen = busy || active;
  ...
  const refresh = useCallback(async () => {
    ...
    const proposal = next.job?.status==="ready" && next.job.kind!=="ideas" && next.job.input_revision===next.state.revision ? next.job.result : null;
  ...
  async function generate(kind:AuthoringJob["kind"]){
    ...
    await flush();
    await post({action:"start",kind,revision:revision.current,requestId:crypto.randomUUID()});await refresh();
  }
  ...
  const proposal=!!data?.job && data.job.status==="ready" && data.job.kind!=="ideas" && data.job.input_revision===data.state.revision;
  const showResult=!!result && !active && (currentStep===6 || (currentStep===5 && proposal));
  const steps=[{id:1,label:"Настройки"},{id:2,label:"Идеи",optional:true},{id:3,label:"Содержание"},...(input.kind==="image"?[{id:4,label:"Картинка"}]:[]),{id:5,label:"Проверка"}];
  ...
  return <section className={styles.wizard} aria-label="Создание публикации через Drafta">
    <header className={styles.header}><div><span className={styles.eyebrow}>ПУБЛИКАЦИИ / DRAFTA</span><h2>{showResult?"Черновик публикации":"Создать публикацию"}</h2></div>
      <button className={styles.secondary} disabled={busy||saving} onClick={()=>void perform(async()=>{await flush();if(resultDirty.current)await saveResult();onClose();})}>Закрыть</button></header>
    ...
    <div className={styles.layout}>
      ...
      {showResult&&result ? <div className={styles.form}>
        <p className={styles.notice}>{proposal?"Проверьте предложенный результат и сохраните черновик. Предыдущая сохранённая версия пока не изменена.":"Черновик сохранён. Публикация в каналы ещё не отправлена."}</p>
        <label>Название черновика<input maxLength={200} value={result.title} onChange={e=>{setResult({...result,title:e.target.value});resultDirty.current=true;}}/></label>
        ...
        {selectedVariant&&<label>Текст публикации<textarea rows={12} maxLength={20000} value={selectedVariant.body} onChange={e=>{setResult({...result,variants:result.variants.map(v=>v.channelId===selectedVariant.channelId?{...v,body:e.target.value}:v)});resultDirty.current=true;}}/></label>}
        {result.assetIds.map(id=><img className={styles.resultImage} key={id} src={`/api/publications/assets/${id}`} alt="Изображение публикации"/>)}
        <div className={styles.actions}><button className={styles.primary} disabled={frozen} onClick={()=>void perform(saveResult)}>Сохранить черновик</button>
          <button className={styles.secondary} disabled={frozen} onClick={()=>void perform(async()=>{await saveResult();await go(5);await generate("text");})}>Перегенерировать текст</button>
          {input.kind==="image"&&<button className={styles.secondary} disabled={frozen} onClick={()=>void perform(async()=>{await saveResult();await go(5);await generate("image");})}>Заменить картинку через AI</button>}
      ...
      {currentStep===1&&<>
        <div><h3>Куда готовим публикацию?</h3><p className={styles.muted}>Выберите один или несколько подключённых каналов.</p>
          ...
          <div className={styles.cards}>{data.channels.map(channel=><label key={channel.id} className={styles.choice} data-selected={input.channelIds.includes(channel.id)}><input type="checkbox" checked={input.channelIds.includes(channel.id)} onChange={()=>toggle("channelIds",channel.id)}/><span><strong>{channel.name}</strong><small>{PLATFORMS[channel.platform]||channel.platform}</small></span></label>)}</div>
        ...
        <div><h3>Тип публикации</h3><div className={styles.cards}>{[{id:"image",name:"Текст + картинка",description:"Один визуал и текст поста"},{id:"carousel",name:"Текст + карусель",description:"Серия слайдов · Скоро"},{id:"text",name:"Только текст",description:"Публикация без изображения"},{id:"video",name:"Сценарий для видео",description:"Для самостоятельной съёмки · Скоро"}].map(type=><label key={type.id} className={styles.choice} data-disabled={!["image","text"].includes(type.id)} data-selected={input.kind===type.id}><input type="radio" name="publication-kind" checked={input.kind===type.id} disabled={!["image","text"].includes(type.id)} onChange={()=>change({...input,kind:type.id as AuthoringInput["kind"]})}/><span><strong>{type.name}</strong><small>{type.description}</small></span></label>)}</div></div>
      ...
      {currentStep===3&&<>
        ...
        {input.kind==="image"&&<label>Формат картинки<select value={input.aspectRatio} onChange={e=>change({...input,aspectRatio:e.target.value as AuthoringInput["aspectRatio"]})}><option value="1:1">1:1 · Квадрат</option><option value="4:5">4:5 · Вертикальный</option><option value="9:16">9:16 · В полный рост</option></select></label>}
        <div className={styles.actions}><button className={styles.secondary} onClick={()=>void perform(()=>go(data.state.ideas.length?2:1))}>Назад</button><button className={styles.primary} disabled={!input.brief.topic.trim()||!input.brief.goal.trim()} onClick={()=>void perform(()=>go(input.kind==="image"?4:5))}>Далее</button></div>
      </>}
      {currentStep===4&&<>
        <div><h3>Картинка для публикации</h3><p className={styles.muted}>Формат {input.aspectRatio}. Одна картинка для выбранных каналов.</p></div>
        <label>Источник картинки<select value={input.image.source} onChange={e=>change({...input,image:{...input.image,source:e.target.value as "generate"|"upload"}})}><option value="generate">Сгенерировать AI</option><option value="upload">Загрузить свою</option></select></label>
      ...
      {(currentStep===5||currentStep===6)&&<>
        ...
        <dl className={styles.summary}><div><dt>Каналы</dt><dd>{input.channelIds.map(id=>data.channels.find(c=>c.id===id)?.name||"Отключённый канал").join(", ")}</dd></div><div><dt>База знаний</dt><dd>{input.kbIds.map(id=>data.categories.find(c=>c.id===id)?.name||"Недоступный элемент").join(", ")||"Без базы знаний"}</dd></div><div><dt>Тип и язык</dt><dd>{input.kind==="image"?"Текст + картинка":"Только текст"} · {input.language}</dd></div>{IDEA_FIELDS.map(f=><div key={f.key}><dt>{f.label}</dt><dd>{input.brief[f.key]||"Не задано"}</dd></div>)}{input.kind==="image"&&<div><dt>Картинка</dt><dd>{input.aspectRatio} · {input.image.source==="upload"?"Своя картинка":input.image.style}</dd></div>}</dl>
        ...
        <div className={styles.actions}><button className={styles.secondary} onClick={()=>void perform(()=>go(input.kind==="image"?4:3))}>Назад</button><button className={styles.primary} disabled={!data.configured} onClick={()=>void perform(()=>generate("post"))}>Сгенерировать пост</button></div>
      </>}
      </fieldset></>}
      </div>
    </div>
