...
export async function GET(request: Request) {
  try {
    ...
    const [drafts, categories, grants, selected, deliveries] = await Promise.all([
      db.from("publication_drafts").select("id,title,body,kind,status,context,asset_ids,edited_at,updated_at,source").eq("workspace_id", workspace.id).order("created_at", { ascending: false }).limit(100),
      ...
      selectedId ? db.from("publication_drafts").select("id,title,body,kind,status,context,asset_ids,edited_at,updated_at,source")
        .eq("workspace_id", workspace.id).eq("id", selectedId).maybeSingle() : Promise.resolve({ data: null, error: null }),
      db.from("publication_deliveries").select("draft_id,channel_id,status,published_url,error").eq("workspace_id",workspace.id),
    ]);
    check(drafts.error); check(categories.error); check(grants.error); check(selected.error); check(deliveries.error);
    return json({ drafts: drafts.data?.map(d=>({...d,deliveries:deliveries.data?.filter(p=>p.draft_id===d.id)??[]})), selectedDraft: selected.data, categories: categories.data, connected: !!grants.data?.length,
...
export async function POST(request: Request) {
  try {
    sameOrigin(request);
    const { db, user, workspace } = await memberContext();
    const source = new URL(request.url).searchParams.get("source") === "draft" ? "draft" : "chatgpt";
    const { data, error } = await db.from("publication_drafts").insert({ workspace_id: workspace.id, created_by: user.id, context: DEFAULT_CONTEXT, source })
      .select("id,title,body,kind,status,context,asset_ids,edited_at,updated_at").single();
    check(error);
    if (source === "draft") {
      const { error: stateError } = await db.from("publication_authoring").insert({ workspace_id: workspace.id, draft_id: data!.id, input: DEFAULT_AUTHORING });
      if (stateError) { await db.from("publication_drafts").delete().eq("workspace_id", workspace.id).eq("id", data!.id); check(stateError); }
    }
