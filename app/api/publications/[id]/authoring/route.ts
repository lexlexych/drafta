...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    if (p.action === "save") {
    ...
    } else if (["start","retry","apply","edit"].includes(p.action)) {
      ...
      if (p.action === "start" || p.action === "retry") {
        ...
        if (!["ideas","post","text","image"].includes(kind)) throw new PublicationError(400, "Недоступный тип генерации.");
      ...
      if (p.action === "edit" && (current.state.input.kind === "text" ? p.result.assetIds.length !== 0 : p.result.assetIds.length !== 1)) throw new PublicationError(400, "Проверьте картинку публикации.");
