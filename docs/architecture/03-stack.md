...
|---|---|---|
| Фронтенд + API | Next.js (App Router, TypeScript) | UI, API-роуты, вебхук-приёмники, код Vercel Workflow-функций |
...
| Фоновые задачи | Vercel Workflow | Генерация черновиков (включая отмену через `getRun().cancel()`), отправка, ручной повтор; push и очистки — до двух повторов. **Правило: в payload событий — только ID, никогда контент** ([§7.2](07-data-flows.md#62-генерация-черновика), [§15](15-compliance-gdpr.md)) |
...
Vercel Workflow дирижирует фоновой работой, но весь исполняемый код живёт в Next.js-проекте
