...
tags: [architecture, stack, nextjs, supabase, workflows, vercel, mistral, postmark, pwa]
chapter: 3
source_section: "2"
type: chapter
up: "[[_index]]"
...
|---|---|---|
| Фронтенд + API | Next.js (App Router, TypeScript) | UI, API-роуты, вебхук-приёмники, код прогонов и их шагов |
...
| Фоновые задачи | Vercel Workflows | Генерация черновиков (включая отмену запущенного прогона по `runId`), отправка, ретраи, кроны. Прогоны пиннятся в `fra1`. **Правило: в аргументах запуска — только ID, никогда контент** ([§7.2](07-data-flows.md#62-генерация-черновика), [§18](18-workflows.md), [§15](15-compliance-gdpr.md)) |
...
Workflows дирижируют фоновой работой, но весь исполняемый код живёт в Next.js-проекте
