...
| Фоновые задачи | Inngest | Генерация черновиков (включая отмену запущенного прогона через `cancelOn`), отправка, ретраи. **Правило: в payload событий — только ID, никогда контент** ([§7.2](07-data-flows.md#62-генерация-черновика), [§15](15-compliance-gdpr.md)) |
