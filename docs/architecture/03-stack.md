...
| LLM | **Mistral (La Plateforme)** | Европейский провайдер, обработка в ЕС, вне американской юрисдикции — ключевое для немецкого рынка. API OpenAI-совместимый: клиент в `lib/ai` остаётся стандартным SDK со сменным `baseURL`, так что альтернативы (Azure OpenAI EU Data Zone, локальная модель) — конфигурация, не код |
