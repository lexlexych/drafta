...
Удалены основной Inngest SDK, serve route, регистрация и dispatch. Все вызывающие пути переведены на Workflow. Обновлены AGENTS, README, конфигурация, архитектура, privacy и инструкции эксплуатации. website/ не изменён. Поиск Inngest в app/lib/package/lock/runtime config не даёт совпадений. План дополнен статусом реализации; переключение production не выполнялось.
