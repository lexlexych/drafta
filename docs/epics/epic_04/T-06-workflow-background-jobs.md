...
Push выполняет отдельный step для подписки, пропускает успешные устройства и удаляет истёкшие подписки. Push и обе очистки имеют maxRetries=2; avatar/thumbnail — ручной повтор. Два Cron защищены CRON_SECRET; пути и UTC-расписания закреплены в vercel.json. Наблюдение облачных terminal failures необходимо настроить по T-08.
