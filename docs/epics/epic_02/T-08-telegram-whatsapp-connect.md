...
> [!note] Статус: WhatsApp закрыт, Telegram остаётся
> **WhatsApp** реализован отдельной веткой `whatsapp-connect` по пути **B1** —
> выяснилось, что специфичного флоу он не требует вовсе: `GET /v1/connect/whatsapp`
> ведёт на Meta Embedded Signup, а остальной контур (callback, парсер, отправка,
> 24-часовое окно) уже был провайдеро-агностичен. Работа свелась к снятию заглушки
> в UI, тексту предусловий, тестам и документации — см.
> [5. Слой абстракции каналов](../../architecture/05-channels.md#whatsapp-свой-номер-а-не-номер-провайдера).
>
> Найденное сверх спеки ниже: покупать номер у Zernio **не нужно** (это их продукт
> для звонков и SMS); номер из приложения WhatsApp Business подключается в режиме
> coexistence; **двухшаговая проверка (PIN) на номере ломает подключение** — Meta
> отвечает ошибкой 133005, лечится снятием PIN либо вызовом
> `POST /v1/accounts/{accountId}/whatsapp/register` (в скоуп не входило).
>
> Не сделано и остаётся здесь: путь **B2** для WABA с несколькими номерами и путь
> **B3** (креденшелы Meta).
>
> **Telegram через Zernio отменён (2026-09-23):** Telegram подключается напрямую через
> Bot API — токен бота от @BotFather, провайдер `telegram`, без Zernio. См.
> [5. Telegram (напрямую, Bot API)](../../architecture/05-channels.md#telegram-напрямую-bot-api).
> Спека Telegram ниже сохранена как история.
