...
Независимо проверены критерии T-05 и фактический diff коммита `a412ab5`.

- RPC `public.create_workspace(text)` — `security definer` с пустым `search_path`;
  пользователь берётся только из `auth.uid()`. В одном вызове выполняются ровно три
  INSERT: `workspaces`, membership с ролью `owner`, `ai_settings`; категорий нет.
  Ошибка любого INSERT откатывает вызов функции как единый PostgreSQL statement.
- У функции отозван PUBLIC EXECUTE и выдан EXECUTE только `authenticated`.
  В конфигурации отключено legacy auto-expose; в миграции T-03 у `authenticated`
  нет ни INSERT ACL, ни INSERT policy для `workspaces`. Прямого INSERT в bootstrap-
  таблицы и secret/admin-клиента в коде T-05 нет.
- Поток и гейты корректны: подтверждённый пользователь без membership проходит
  `/dashboard` → `/onboarding` → RPC → `/dashboard`; пользователь с workspace
  пропускает onboarding; неавторизованный направляется на `/login`. Цикла редиректов
  для этих состояний нет. Dashboard читает данные через server-клиент с RLS и выводит
  имя workspace, email и роль; onboarding использует только browser-клиент.
- Терминология, scope и правила вайбкодинга соблюдены: `account` в исходниках T-05
  отсутствует; внешние сервисы, вебхуки, Inngest, LLM и новые секреты не добавлены.
- Cloud-проверка двумя пользователями не выполнялась по прямому запрету не использовать
  Docker и внешние ресурсы. Она корректно зафиксирована как обязательная ручная
  проверка в T-08: шаги 2a и 7a, а также «Финальная проверка эпика», пункты 1–3.
  До финализации эпика человеку нужно выполнить этот сценарий в двух браузерных
  профилях после применения миграций.

Прогнано повторно:

- `git diff --check 73be508..HEAD` — успешно.
- Независимая статическая PowerShell-проверка миграций, ACL/RLS и клиентских границ —
  успешно: все инварианты RPC найдены, INSERT ровно 3, категорий/secret-клиента/
  прямого INSERT нет.
- `npm.cmd run lint` — успешно.
- `npx.cmd tsc --noEmit` — успешно.
- `npm.cmd test` — успешно: 7 файлов, 16 тестов.
- `npm.cmd run build` — успешно; `/onboarding` и `/dashboard` собраны как динамические SSR-маршруты.
- `supabase db reset` не запускалась: команда требует Docker, который прямо запрещён
  для этой проверки; Cloud Supabase/Vercel/SMTP также намеренно не затрагивались.
